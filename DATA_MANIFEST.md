...
## 27. Initial Control Catalog

This is a proposed first set of deterministic controls. Names should be stable
because they become baseline keys, LLM references, and test fixtures.

### Google Workspace User Controls

| Control ID | Entity | Inputs | PASS when | UNKNOWN when |
|---|---|---|---|---|
| `gws.user.mfa_enrolled` | `GWSUser` | `mfa_enabled` | value is `true` | value is nil |
| `gws.user.mfa_enforced` | `GWSUser` | `mfa_enforced` | value is `true` | value is nil |
| `gws.user.admin_review` | `GWSUser` | `is_admin` | value matches class expectation | value is nil |
| `gws.user.oauth_tokens_within_limit` | `GWSUser` | `third_party_tokens` | count <= policy limit | value is nil |
| `gws.user.no_app_passwords` | `GWSUser` | `asp_count` | count == 0 | value is nil |
| `gws.user.no_backup_codes` | `GWSUser` | `backup_code_count` | count == 0 | value is nil |

### JumpCloud User Controls

| Control ID | Entity | Inputs | PASS when | UNKNOWN when |
|---|---|---|---|---|
| `jc.user.mfa_enabled` | `JCUser` | `mfa_enabled` | value is `true` | value is nil |
| `jc.user.password_expires` | `JCUser` | `password_never_expires` | value is `false` | value is nil |
| `jc.user.jumpcloud_go_enabled` | `JCUser` | `jumpcloud_go_enabled` | value is `true` when required | value is nil |
| `jc.user.not_suspended` | `JCUser` | `suspended` | value is `false` for active class | value is nil |
| `jc.user.not_locked` | `JCUser` | `account_locked` | value is `false` | value is nil |

### JumpCloud Device Controls

| Control ID | Entity | Inputs | PASS when | UNKNOWN when |
|---|---|---|---|---|
| `jc.device.disk_encryption` | `JCDevice` | `disk_encrypted`, `encryption_type` | `disk_encrypted == true` | `disk_encrypted` is nil |
| `jc.device.mdm_enrollment` | `JCDevice` | `mdm_enrolled`, `mdm_vendor` | `mdm_enrolled == true` | value is nil |
| `jc.device.agent_active` | `JCDevice` | `active`, `last_contact` | active and recently contacted | inputs missing |
| `jc.device.no_unexpected_local_users` | `JCDevice` | `unexpected_local_users` | list is empty | list unavailable |
| `jc.device.policy_failures_absent` | `jumpcloud.System` or `JCPolicyEnforcement` | policy statuses | no failed/error policies | policy status unavailable |

### Sophos Endpoint Controls

| Control ID | Entity | Inputs | PASS when | UNKNOWN when |
|---|---|---|---|---|
| `sophos.endpoint.tamper_protection` | `SophosEndpoint` | `tamper_protection` | value is `true` | value is nil |
| `sophos.endpoint.health_good` | `SophosEndpoint` | `health_overall` | value is `good` | empty/unknown |
| `sophos.endpoint.no_open_alerts` | `SophosEndpoint` | `alert_count` | count == 0 | alerts unavailable |
| `sophos.endpoint.detections_available` | `SophosEndpoint` | `fetch_error` | value is `false` | value is true/unknown |

### SaaS Controls / Signals

SaaS should probably start as `signals.json` rather than drift findings.

| Signal ID | Entity | Inputs | Risk when |
|---|---|---|---|
| `saas.app.unapproved` | `SaaSApp` | `status` | status is `UNAPPROVED` |
| `saas.app.newly_discovered` | `SaaSApp` | `status` | status is `NEWLY_DISCOVERED` |
| `saas.app.sso_not_connected` | `SaaSApp` | `sso_connected` | false for app categories where SSO is expected |
| `saas.app.unused_licenses` | `SaaSApp` | license totals | unassigned seats > 0 |
| `saas.account.unattributed` | `SaaSAccount` | email/username/device owner | no usable owner identity |

### PeopleForce Controls / Signals

PeopleForce can also start as signals until the team decides whether asset
assignment should be baseline drift.

| Signal ID | Entity | Inputs | Risk when |
|---|---|---|---|
| `pf.asset.unassigned` | `PFAsset` | `is_assigned` | false for managed device categories |
| `pf.asset.missing_serial` | `PFAsset` | `serial_number` | empty serial |
| `pf.asset.owner_missing_email` | `PFAsset` | `assigned_to_email` | assigned but email empty |
| `pf.asset.duplicate_serial` | `PFAsset` | normalized serial index | serial appears on multiple assets |

## 28. LLM Output Contract for Controls

The LLM-facing control data should be compact and explanation-ready.

Recommended JSONL record:

```json
{
  "control_id": "jc.device.disk_encryption",
  "status": "FAIL",
  "severity": "CRIT",
  "entity_id": "jc:device:abc123",
  "entity_type": "device",
  "owner_email": "person@example.com",
  "hostname": "laptop-01",
  "actual": "false",
  "expected": "true",
  "reason": "JumpCloud reports disk encryption disabled",
  "evidence_refs": [
    "local/current/snapshot.json#jumpcloud.devices[system_id=abc123].disk_encrypted"
  ]
}
```

The LLM should be instructed to prefer these records over re-deriving posture
from raw fields. If a user asks "why", the LLM follows `evidence_refs`.

Recommended LLM entrypoints after this layer exists:

| Question type | First file |
|---|---|
| "What is wrong today?" | `llm/digest.json`, then `llm/controls.jsonl` |
| "Is this user secure?" | `llm/users.jsonl`, then controls by entity ID |
| "Is this laptop compliant?" | `llm/devices.jsonl`, then controls by device/person |
| "What changed?" | `diff/summary.json`, then `diff/field_changes.jsonl` |
| "Which data is unreliable?" | `quality_report.json`, then UNKNOWN controls |

## 29. Updated Implementation Plan

This plan assumes no collector rewrite first. It builds a deterministic analysis
layer on top of the existing artifacts, then wires it into drift and LLM data.

### Phase 1: Run and Artifact Foundations

1. Add `run_id` generation.
2. Add immutable `local/runs/<date>/<run_id>/` writes.
3. Keep `current/` as latest mirror.
4. Keep or redefine `daily/<date>/` as latest successful run for that day.
5. Add `run_manifest.json` with schema versions, artifact hashes, counts,
   command flags, build info, collection scope, and completeness.

### Phase 2: Quality and Normalization

1. Add shared key normalization helpers for email, serial, hostname, MAC, and
   domain.
2. Persist normalized keys in derived artifacts.
3. Add `quality_report.json`.
4. Add quality checks for missing IDs, duplicate serials, weak joins, nil
   monitored fields, skipped collectors, and partial runs.

### Phase 3: Programmatic Controls

1. Create a control model package with `ControlDefinition`, `ControlResult`,
   `FieldInput`, and `PostureSummary`.
2. Implement evaluators for existing classified entities first:
   `GWSUser`, `JCUser`, `JCDevice`, `SophosEndpoint`.
3. Persist `controls.json` and `llm/controls.jsonl`.
4. Add unit tests for PASS, FAIL, UNKNOWN, and NOT_APPLICABLE behavior.
5. Add posture summaries per entity.

### Phase 4: Drift Integration

1. Extend baseline config with optional `expected_controls`.
2. Keep existing `expected` field comparisons for backwards compatibility.
3. Make drift compare expected controls to `ControlResult.Status`.
4. Map `FAIL` to `BASELINE_DRIFT`.
5. Map `UNKNOWN` or missing control result to `DATA_GAP`.
6. Point findings to control evidence first, source fields second.

### Phase 5: Signals for Non-Drift Risk

1. Add `signals.json` for SaaS, PeopleForce, stale devices, weak joins, and
   license waste.
2. Keep `FindingKind` closed unless there is an explicit schema decision.
3. Add `llm/signals.jsonl`.
4. Add digest or LLM manifest pointers to signals.

### Phase 6: LLM Shards and Indexes

1. Add `llm_manifest.json`.
2. Add entity JSONL shards for users, devices, service-specific records, SaaS,
   PeopleForce assets, findings, controls, and signals.
3. Add indexes by email, serial, hostname, service ID, control ID, severity,
   SaaS status, and owner.
4. Add token-size estimates per shard.
5. Add a redaction profile for LLM exports.

### Phase 7: Diffs and Trend Context

1. Identify previous successful full run from `run_manifest.json`.
2. Generate entity-added, entity-removed, and field-changed diff artifacts.
3. Generate control status changes, for example PASS -> FAIL or UNKNOWN -> PASS.
4. Add compact `diff/summary.json` for LLM first-pass context.

### Phase 8: Reporting and Operator UX

1. Surface control counts in logs and summaries.
2. Add Sheets or digest support for high-severity control failures if useful.
3. Add `inventory validate` or equivalent dry-run command for quality/control
   checks without publishing.
4. Document the LLM retrieval contract so agents read manifests first, then
